...
public class LoadMessagesThread {
	...
	/**
	 * Thread which loads the messages from the server sleeps 250ms
	 */
			...
			try {
				...
				for (int start = serverMessageList.size() - difference; start <= end; start++) {
					...
					if (serverMessageList.get(start).getSender().getName().equals(user.getName())) {
						message = serverMessageList.get(start).getMsg() + " | "
								+ serverMessageList.get(start).getSender().getName() + " "
								+ serverMessageList.get(start).getTime().getHours() + ":"
								+ serverMessageList.get(start).getTime().getMinutes();
						...
						message = serverMessageList.get(start).getTime().getHours() + ":"
								+ serverMessageList.get(start).getTime().getMinutes() + " "
								+ serverMessageList.get(start).getSender().getName() + " | "
								+ serverMessageList.get(start).getMsg();
